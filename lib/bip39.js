 ...
 * from a wordlist), and then that mnemonic into a seed for a private key or
 * BIP32 master private key.  It does not go the other way around (i.e., you
 * cannot turn a private key into a string of words). The usual way to use it
 * is:
 * var mnemonic = BIP39.mnemonic();
 * var seed = BIP39.mnemonic2seed(passphrase);
 */
...
/**
 * Generate a random new mnemonic with an optional wordlist.
 */
BIP39.mnemonic = function(bits, wordlist) {
  ...
  return BIP39.entropy2mnemonic(buf, wordlist);
}

/**
 * Generate a new mnemonic (a string of words from a wordlist) from some
 * entropy generated somewhere else. The entropy must be 128 bits or 256 bits.
 */
BIP39.entropy2mnemonic = function(buf, wordlist) {
  if (typeof wordlist === 'undefined')
    wordlist = BIP39.wordlist_en;
 ...
 */
BIP39.check = function(mnemonic, wordlist) {
  if (typeof wordlist === 'undefined')
    wordlist = BIP39.wordlist_en;
 ...
 */
BIP39.mnemonic2seed = function(mnemonic, passphrase, wordlist) {
  if (!BIP39.check(mnemonic, wordlist))
    throw new Error('Mnemonic does not pass the check - was the mnemonic typed incorrectly? Are there extra spaces?');
  if (!passphrase)
    passphrase = "";
  var passbuf = Buffer.concat([new Buffer('mnemonic'), new Buffer(passphrase, 'utf8')]);
  var buf = new Buffer(mnemonic, 'utf8');
  var buf = KDF.PBKDF2(new Buffer(mnemonic, 'utf8'), passbuf, 2048, 64 * 8);
  return buf;
}
